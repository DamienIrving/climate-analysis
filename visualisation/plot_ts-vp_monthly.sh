...
if [ ${contour_type} = 'raw' ] ; then
    ...
    vp_file=${inpath}/processed/vp_Merra_250hPa_monthly_native.nc
    uad_file=${inpath}/processed/uad_Merra_250hPa_monthly_native.nc
    vad_file=${inpath}/processed/vad_Merra_250hPa_monthly_native.nc
    cbar=RdBu_r
    pticks='-5,-4.5,-4,-3.5,-3,-2.5,-2,-1.5,-1,-0.5,0,0.5,1,1.5,2,2.5,3,3.5,4,4.5,5'
    sticks='-9,-7.5,-6,-4.5,-3,-1.5,0,1.5,3,4.5,6,7.5,9'
    title='250hPa_velocity_potential_&_SST_anomaly'
    outfile_start=${outpath}/ts-vp_Merra_surface-250hPa_monthly_native
elif [ ${contour_type} = 'anomaly' ] ; then
    ts_file=${inpath}/processed/ts_Merra_surface_monthly-anom-wrt-1981-2010_native-ocean.nc
    vp_file=${inpath}/processed/vp_Merra_250hPa_monthly-anom-wrt-1981-2010_native.nc
    uad_file=${inpath}/processed/uad_Merra_250hPa_monthly-anom-wrt-1981-2010_native.nc
    vad_file=${inpath}/processed/vad_Merra_250hPa_monthly-anom-wrt-1981-2010_native.nc
    cbar=RdBu_r
    pticks='-5,-4.5,-4,-3.5,-3,-2.5,-2,-1.5,-1,-0.5,0,0.5,1,1.5,2,2.5,3,3.5,4,4.5,5'
    sticks='-9,-7.5,-6,-4.5,-3,-1.5,0,1.5,3,4.5,6,7.5,9'
    title='250hPa_velocity_potential_anomaly_&_SST_anomaly'
    outfile_start=${outpath}/ts-vp_Merra_surface-250hPa_monthly-anom-wrt-1981-2010_native
else
...
do
	...
	/opt/cdat/bin/cdat plot_map.py temp_ts.nc ts --title ${title} --contour --ticks ${pticks} --draw_contours --contour_files temp_vp.nc --contour_variables vp --contour_ticks ${sticks} --image_size 13 --draw_axis --ofile ${outfile} --img_headings ${m}${y} --units Celsius --extend both --colourbar_colour ${cbar} --enso --projection ${proj} --contour_scale 1.e-6 --draw_vectors --uwnd_files temp_uad.nc --uwnd_variables uad --vwnd_files temp_vad.nc --vwnd_variables vad --thin 10
	echo ${outfile}
        rm temp_ts.nc
	rm temp_vp.nc
	rm temp_uad.nc
