...
nargs=5
function usage {
    echo "USAGE: $0 n var var_long input output"
    echo "    -r:         Select region [options: eqpacific]"
    echo "    -s:         Select start date [e.g. 1979-01-01]"
    echo "    -e:         Select end date [e.g. 2010-12-31]"
...
# Parse the command line for required arguments
...
if (( ${region_flag} )) && (( ${end_flag} )) ; then
  $MYDIR/named_region.sh ${region} ${infile} ${tmpregfile}
  cdo seldate,${start_date},${end_date} ${tmpregfile} ${tmptimefile}
  infile=${tmptimefile}
...
for n in `seq ${np}`; do
  ...
  for var in ${vars} ; do
    ...
    ncatted -h -a standard_name,${var}_ts_eof${n},o,c,"eof_of_${varlong}" ${tmpcdoeofcoeff}${nf}_nospace.nc
    ncatted -h -a name,${var}_ts_eof${n},o,c,"eof_of_${varlong}" ${tmpcdoeofcoeff}${nf}_nospace.nc
    ...
    ncatted -h -a standard_name,${var}_spatial_eof${n},o,c,"eof_of_${varlong}" ${tmpcdoeof2}_${n}.nc
    ncatted -h -a name,${var}_spatial_eof${n},o,c,"eof_of_${varlong}" ${tmpcdoeof2}_${n}.nc
    ...
    ncatted -h -a standard_name,${var}_perc_eof${n},o,c,"eof_of_${varlong}" ${tmpperval}
    ncatted -h -a name,${var}_perc_eof${n},o,c,"eof_of_${varlong}" ${tmpperval}
...
rm ${tmpcdoeof1} ${tmpcdoeof2} ${tmppercvar} ${griddes}
