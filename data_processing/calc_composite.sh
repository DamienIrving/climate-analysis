...
for var in tas ; do
    for region in marie-byrd-land antarctic-peninsula ; do
        for season in DJF MAM JJA SON ; do
            ...
            /usr/local/uvcdat/1.3.0/bin/cdat calc_composite.py ${infile} ${var} ${data_path}/stats/${base}-${region}-va-below-neg5.txt ${data_path}/stats/${var}-${base}-${region}-va-below-neg5_composite-${season}.nc --time 1979-01-01 2012-12-31 ${season}
